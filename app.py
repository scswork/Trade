...
if df_loaded.empty:
    ...
    if st.sidebar.button("Apply Filters"):
        ...
        if df_filtered.empty:
            ...
            # -------------------- HHI Calculation (filtered data only) --------------------
            hhi_calc = (
                df_filtered.groupby(['HS10','SUPC','Country'], as_index=False)['Value'].sum()
            )
            total_per_product = (
                hhi_calc.groupby(['HS10','SUPC'], as_index=False)['Value'].sum().rename(columns={'Value':'total_value'})
            )
            hhi_calc = hhi_calc.merge(total_per_product, on=['HS10','SUPC'])
            hhi_calc['share_sq'] = (hhi_calc['Value'] / hhi_calc['total_value'])**2
            hhi_table = (
                hhi_calc.groupby(['HS10','SUPC'], as_index=False)['share_sq'].sum().rename(columns={'share_sq':'HHI'})
            )
...
PCI (2023): {pci_value}
Aggregate HHI (filtered data): {prod['HHI']:.4f}
"""
                st.text(summary_text)

                # Top 10 countries by Value for this product in filtered data
                top_countries = (
